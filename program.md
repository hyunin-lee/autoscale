...
In addition to optimizing the final `val_bpb`, you should actively create custom monitoring metrics inside `train.py`. `val_bpb` is a single end-of-run scalar: it tells you *whether* your last change helped, never *why* or *what to change next*. Each monitoring metric should reveal the training curve and dynamics in more depth than that terminal number can — but that depth is a means, not the end. It is worthwhile only when it sharpens your *next* decision: which hyperparameter is currently limiting quality, and which way to move it. The goal remains the lowest final `val_bpb`, reached through better-informed edits rather than blind trial-and-error; insight that never changes an edit is decoration.
